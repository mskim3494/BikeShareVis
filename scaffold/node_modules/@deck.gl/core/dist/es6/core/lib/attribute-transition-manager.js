...
const Transform = experimental.Transform;
...
export default class AttributeTransitionManager {
  ...
  _getTransitionSettings(attribute) {
    ...
    const transition = attribute.transition,
  ...
  _triggerTransition(transition, settings) {
    ...
    const needsNewBuffer = !buffer || transition.bufferSize < value.length; // Attribute descriptor to transition from
    // _getCurrentAttributeState must be called before the current buffer is deleted

    const fromState = this._getCurrentAttributeState(transition); // Attribute descriptor to transition to
    // Pre-converting to buffer to reuse in the case where no transition is needed


    const toState = new Buffer(this.gl, {
      size,
      data: value
    });
