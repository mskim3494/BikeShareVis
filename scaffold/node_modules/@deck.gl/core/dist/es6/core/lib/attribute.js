...
export default class Attribute {
  ...
  finalize() {} // TODO call buffer.finalize();
  ...
  allocate(numInstances) {
    ...
    const attribute = this.state; // Allocate a new typed array if needed
  ...
  _updateBufferViaStandardAccessor(data, props) {
    ...
    let _attribute$defaultVal = attribute.defaultValue,
  ...
  _validateAttributeDefinition() {
    const attribute = this.state;
    assert(attribute.size >= 1 && attribute.size <= 4, `Attribute definition for ${this.id} invalid size`);
  } // Validate deck.gl level fields


  _validateAttributeUpdaters() {
    const attribute = this.state; // Check that either 'accessor' or 'update' is a valid function

    const hasUpdater = attribute.noAlloc || typeof attribute.update === 'function' || typeof attribute.accessor === 'string';
