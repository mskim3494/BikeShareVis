...
const LOG_PROGRAM_PERF_PRIORITY = 3; // const GL_TRANSFORM_FEEDBACK_BUFFER_MODE = 0x8C7F;
...
export default class Program extends Resource {
  ...
  checkAttributeBindings({
    ...
    const filledLocations = vertexArray ? vertexArray.filledLocations : this.vertexAttributes.filledLocations;
  ...
  setBuffers(buffers, {
    ...
    const _sortBuffersByLocatio = this._sortBuffersByLocation(buffers),
    ...
    for (let location = 0; location < locations.length; ++location) {
      ...
      const buffer = buffers[bufferName]; // DISABLE MISSING ATTRIBUTE
  ...
  _sortBuffersByLocation(buffers) {
    let elements = null;
    let locations = []; // Reutrn early if no buffers to be bound.
  ...
  _setId(id) {
    ...
    if (!id) {
      const programName = this._getName(); // TODO - this.id will already have been initialized
