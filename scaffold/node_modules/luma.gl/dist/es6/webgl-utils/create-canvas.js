...
const pageLoadPromise = isPage ? new Promise((resolve, reject) => {
  if (isPageLoaded) {
    resolve(document);
    return;
  }
...
export function calculateDrawingBufferSize(canvas, options) {
  let _options$useDevicePix = options.useDevicePixels,
  ...
  const cssToDevicePixels = useDevicePixels ? window.devicePixelRatio || 1 : 1; // Lookup the size the browser is displaying the canvas in CSS pixels
...
export function resizeDrawingBuffer(canvas, {
  ...
  const newBufferSize = calculateDrawingBufferSize(canvas, {
    useDevicePixels
